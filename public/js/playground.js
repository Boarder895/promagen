...
function showError(e) {
  console.error(e);
  msg.textContent = (e?.message || String(e)).replace(/^"|"$/g, "");
...
async function loadProviders() {
  try {
    ...
    providerSel.innerHTML = providers.map(p => `<option value="${p.id}">${p.label || p.id}</option>`).join("");
...
async function generate(e) {
  ...
  try {
    ...
    localStorage.setItem("pmg.provider", provider);
  } catch (err) {
    showError(err);
  } finally {
    go.disabled = false;
...
async function clearGallery() {
  ...
  go.disabled = true;
  try {
    await j("/api/ai/images/clear", { method: "POST" });
    await loadStats();
    await loadImages();
