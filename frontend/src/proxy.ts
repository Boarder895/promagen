...
const CANONICAL_HOST = (process.env.NEXT_PUBLIC_CANONICAL_HOST ?? "")
  .trim()
  .toLowerCase();
const ENFORCE_CANONICAL =
  (process.env.NEXT_PUBLIC_ENFORCE_CANONICAL ?? "").trim() === "1";
const DEV_HEALTH_TOKEN = (process.env.PROMAGEN_DEV_HEALTH_TOKEN ?? "").trim();

const DEV_HEALTH_PATHS = new Set<string>([
  "/dev/health",
  "/api/health",
  "/health-check",
]);
...
function buildCsp(isDev: boolean, isPreview: boolean): string {
  ...
  directives.push(
    `img-src 'self' data: blob: https://img.clerk.com https://img.clerk.dev`,
  );
  ...
  directives.push(
    `connect-src 'self' https:${isDev ? " http: ws: wss:" : ""} ${clerkConnectSrc}`,
  );
...
const devMiddleware = clerkMiddleware(async (_auth, req) => {
  ...
  if (isDevHealth) {
    ...
    const allowed =
      (DEV_HEALTH_TOKEN && token === DEV_HEALTH_TOKEN) ||
      isPromagenInternalHost(host);
    if (!allowed) {
