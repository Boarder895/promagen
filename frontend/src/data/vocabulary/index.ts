 ...
 */
export function getTemperaturePhrase(tempC: number, context?: WeatherContext, seed?: number): string {
...
export function getContextualTemperaturePhrase(contextKey: string, seed?: number): string {
  const contextual = (temperatureData.contextual as Record<string, string[]>)[contextKey];
 ...
 */
export function getHumidityPhrase(humidity: number, context?: WeatherContext, seed?: number): string {
...
export function getContextualHumidityPhrase(contextKey: string, seed?: number): string {
  const contextual = (humidityData.contextual as Record<string, string[]>)[contextKey];
  if (!contextual || contextual.length === 0) return '';
  return selectFromPool(contextual, seed ?? Date.now());
}
...
export function getContextualWindPhrase(contextKey: string, seed?: number): string {
  const contextual = (windData.contextual as Record<string, string[]>)[contextKey];
  if (!contextual || contextual.length === 0) return '';
  return selectFromPool(contextual, seed ?? Date.now());
}
...
const timePeriods = timeOfDayData.periods as Record<string, TimePeriod>;
 ...
 */
export function getTimePhrase(hour: number, context?: WeatherContext, seed?: number): TimeMoodLighting {
 ...
 */
export function getContextualTimePhrase(contextKey: string, seed?: number): TimeMoodLighting | null {
  const contextual = (timeOfDayData.contextual as Record<string, TimeMoodLighting[]>)[contextKey];
...
const cityVibes = cityVibesData.cities as Record<string, CityVibes>;
...
export function getCityVibe(city: string, seed?: number): string {
  ...
  for (const [key, value] of Object.entries(cityVibes)) {
    if (cityLower.includes(key)) {
      return selectFromPool(value.phrases, seed ?? cityLower.charCodeAt(0) * 1.5);
    }
  }
...
export function getContextualCityPhrase(contextKey: string, seed?: number): string {
  const contextual = (cityVibesData.contextual as Record<string, string[]>)[contextKey];
  if (!contextual || contextual.length === 0) return '';
  return selectFromPool(contextual, seed ?? Date.now());
}
 ...
 */
export function generateWeatherPhrases(input: WeatherPhraseInput, context?: WeatherContext): WeatherPhraseOutput {
...
export function getVocabularyStats(): VocabularyStats {
  ...
  const conditionCount = Object.values(conditionTypes).reduce((sum, c) => sum + c.phrases.length, 0);
  const cityCount = Object.values(cityVibes).reduce((sum, c) => sum + c.phrases.length, 0);
