...
// Shape tests for providers.json – guards the canonical 40-provider catalogue.
...
describe("providers.catalog.json shape", () => {
  it("has the canonical 40 providers (update intentionally if this changes)", () => {
  ...
  it("every provider has required fields with correct basic types", () => {
    ...
    providers.forEach((provider: ProviderRecord) => {
      ...
      expect(
        Object.prototype.hasOwnProperty.call(provider, "affiliateUrl"),
      ).toBe(true);
      expect(
        provider.affiliateUrl === null ||
          typeof provider.affiliateUrl === "string",
      ).toBe(true);
