  ...
  return (
    <span className={className ?? 'inline-flex items-center'} {...wrapperProps}>
      {src ? (
        ...
        // Emoji fallback: explicit sizing keeps emoji flags visually
        // consistent with SVG flags. Emojis are text glyphs, so we
        // control their size via font-size + constrained box.
        <span
          title={tooltipText}
