...
import { pingHealth, type PingHealth } from "@/lib/ping";

type View =
  | { status: "ok" | "degraded"; latency?: number }
  | { status: "down"; latency?: number; detail?: string }
  | { status: "unset" };
...
export default function ServiceBanner() {
  ...
  useEffect(() => {
    ...
    async function check() {
      if (!apiBase) {
        setView({ status: "unset" });
        return;
      }
      const h: PingHealth = await pingHealth(3500);
      if (!on) return;
  ...
  return (
    ...
    >
      <strong>{label}</strong>
      {"latency" in view && typeof view.latency === "number" ? ` • ${view.latency}ms` : ""}
      {"detail" in view && view.detail ? ` — ${view.detail}` : ""}
      <button
