// src/components/ribbon/crypto-ribbon.tsx
//
// Presentational Crypto ribbon row (API Brain compliant).
...
export interface CryptoRibbonChip {
  id: string;
...
export interface CryptoRibbonProps {
  chips: CryptoRibbonChip[];
}
...
export default function CryptoRibbon({ chips }: CryptoRibbonProps) {
  ...
  const renderChip = (chip: CryptoRibbonChip, keyPrefix?: string) => {
    const priceClasses = [
      'tabular-nums',
  ...
  return (
    <section
      aria-label="Cryptocurrency snapshot"
      data-testid="crypto-ribbon"
