// src/lib/optimise-prompts/group-nl-pixlr.ts
// ============================================================================
// DEDICATED BUILDER: Pixlr — Independent System Prompt
// ============================================================================
// Tier 3 | idealMin 50 | idealMax 200 | maxChars 500 | Strategy: REFINE
// negativeSupport: none
// architecture: proprietary
//
// Platform knowledge: Three generation modes (Fast/Pro/Ultra). Negative prompt field available. NL pro...
//
...
// ============================================================================
// COMPLIANCE: Pixlr-specific cleanup
// ============================================================================

function enforcePixlrCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildPixlrPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "Pixlr". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforcePixlrCleanup,
  };
}
