// src/lib/optimise-prompts/group-nl-myedit.ts
// ============================================================================
// DEDICATED BUILDER: MyEdit — Independent System Prompt
// ============================================================================
// Tier 3 | idealMin 50 | idealMax 200 | maxChars 500 | Strategy: REFINE
// negativeSupport: none
// architecture: proprietary
//
// Platform knowledge: Nano Banana Pro (Google) backend. NL prose. No special syntax.
//
...
// ============================================================================
// COMPLIANCE: MyEdit-specific cleanup
// ============================================================================

function enforceMyeditCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildMyeditPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "MyEdit". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforceMyeditCleanup,
  };
}
