// src/lib/optimise-prompts/group-nl-bluewillow.ts
// ============================================================================
// DEDICATED BUILDER: BlueWillow — Independent System Prompt
// ============================================================================
// Tier 4 | idealMin 50 | idealMax 200 | maxChars 6000 | Strategy: REFINE
// negativeSupport: inline
// architecture: natural-language
//
// Platform knowledge: Discord-based interface. Very limited parameters (--ar, --no only). Short simple...
//
...
// ============================================================================
// COMPLIANCE: BlueWillow-specific cleanup
// ============================================================================

function enforceBluewillowCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildBluewillowPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "BlueWillow". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforceBluewillowCleanup,
  };
}
