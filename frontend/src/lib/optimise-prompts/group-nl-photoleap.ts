// src/lib/optimise-prompts/group-nl-photoleap.ts
// ============================================================================
// DEDICATED BUILDER: Photoleap — Independent System Prompt
// ============================================================================
// Tier 4 | idealMin 100 | idealMax 250 | maxChars 500 | Strategy: BALANCE
// negativeSupport: none
// architecture: proprietary
//
// Platform knowledge: Mobile-first SD-based platform. NL prose. No special syntax.
//
...
// ============================================================================
// COMPLIANCE: Photoleap-specific cleanup
// ============================================================================

function enforcePhotoleapCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildPhotoleapPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "Photoleap". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforcePhotoleapCleanup,
  };
}
