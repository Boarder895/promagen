// src/lib/optimise-prompts/group-nl-picsart.ts
// ============================================================================
// DEDICATED BUILDER: PicsArt — Independent System Prompt
// ============================================================================
// Tier 4 | idealMin 100 | idealMax 300 | maxChars 500 | Strategy: BALANCE
// negativeSupport: none
// architecture: proprietary
//
// Platform knowledge: 90+ models via AI Playground. NL prose. No special syntax.
//
...
// ============================================================================
// COMPLIANCE: PicsArt-specific cleanup
// ============================================================================

function enforcePicsartCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildPicsartPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "PicsArt". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforcePicsartCleanup,
  };
}
