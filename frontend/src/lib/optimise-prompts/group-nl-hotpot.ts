// src/lib/optimise-prompts/group-nl-hotpot.ts
// ============================================================================
// DEDICATED BUILDER: Hotpot AI — Independent System Prompt
// ============================================================================
// Tier 4 | idealMin 100 | idealMax 300 | maxChars 500 | Strategy: BALANCE
// negativeSupport: none
// architecture: proprietary
//
// Platform knowledge: 180+ style presets do the creative work. SDXL backbone abstracted away. 380-char...
//
...
// ============================================================================
// COMPLIANCE: Hotpot AI-specific cleanup
// ============================================================================

function enforceHotpotCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildHotpotPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "Hotpot AI". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
LENGTH: 100–300 characters. Going OVER 300 is a HARD FAILURE. Count your characters BEFORE returning JSON.\n180+ style presets do creative work. SDXL backbone abstracted. Keep prompt focused on scene content.

OUTPUT REQUIREMENTS:
- Flowing natural language prose, 3–4 sentences
- Front-load the primary subject in the first 10 words
- 100–300 characters (HARD CEILING: 300)
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforceHotpotCleanup,
  };
}
