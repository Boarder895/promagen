// src/lib/optimise-prompts/group-nl-craiyon.ts
// ============================================================================
// DEDICATED BUILDER: Craiyon — Independent System Prompt
// ============================================================================
// Tier 4 | idealMin 50 | idealMax 150 | maxChars 500 | Strategy: REFINE
// negativeSupport: none
// architecture: transformer
//
// Platform knowledge: 9 images per generation. Pipe | syntax for concept combining. Raw mode available...
//
...
// ============================================================================
// COMPLIANCE: Craiyon-specific cleanup
// ============================================================================

function enforceCraiyonCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
  ...
  const CEILING = 375;  // maxChars from platform-config.json
...
export function buildCraiyonPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "Craiyon". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
LENGTH RULES:
HARD: Do not shorten any prompt that is below ${ctx.maxChars ?? 375} characters.
SOFT: You may lengthen the prompt up to ${ctx.maxChars ?? 375} characters, but only if the added content is a genuine visual anchor — not filler.
Your job is to produce the best possible prompt for this platform. Length is not a goal. Anchor preservation is.\nSupports pipe | syntax for concept combining. 9 images per generation. Very short prompts work best.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforceCraiyonCleanup,
  };
}
