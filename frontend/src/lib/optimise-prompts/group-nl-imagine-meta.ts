// src/lib/optimise-prompts/group-nl-imagine-meta.ts
// ============================================================================
// DEDICATED BUILDER: Meta Imagine — Independent System Prompt
// ============================================================================
// Tier 3 | idealMin 200 | idealMax 400 | maxChars 1000 | Strategy: ENRICH
// negativeSupport: none
// architecture: transformer
//
// Platform knowledge: Meta Emu model. NL prose. No special syntax. Square output only.
//
...
// ============================================================================
// COMPLIANCE: Meta Imagine-specific cleanup
// ============================================================================

function enforceImagineMetaCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildImagineMetaPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "Meta Imagine". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
LENGTH: 200–400 characters. You have room for richness — use it, but respect the ceiling. Count your characters BEFORE returning JSON.

OUTPUT REQUIREMENTS:
- Flowing natural language prose, 3–4 sentences
- Front-load the primary subject in the first 10 words
- 200–400 characters (HARD CEILING: 400)
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforceImagineMetaCleanup,
  };
}
