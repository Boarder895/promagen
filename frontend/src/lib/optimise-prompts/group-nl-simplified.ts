// src/lib/optimise-prompts/group-nl-simplified.ts
// ============================================================================
// DEDICATED BUILDER: Simplified — Independent System Prompt
// ============================================================================
// Tier 3 | idealMin 100 | idealMax 300 | maxChars 500 | Strategy: BALANCE
// negativeSupport: none
// architecture: proprietary
//
// Platform knowledge: 14+ model aggregator. NL prose across all models. Negative prompts on some model...
//
...
// ============================================================================
// COMPLIANCE: Simplified-specific cleanup
// ============================================================================

function enforceSimplifiedCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildSimplifiedPrompt(
  ...
  const systemPrompt = `You are an expert prompt optimiser for "Simplified". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforceSimplifiedCleanup,
  };
}
