// src/lib/optimise-prompts/group-nl-jasper-art.ts
// ============================================================================
// DEDICATED BUILDER: Jasper Art — Independent System Prompt
// ============================================================================
// Tier 4 | idealMin 200 | idealMax 400 | maxChars 1000 | Strategy: ENRICH
// negativeSupport: none
// architecture: natural-language
//
// Platform knowledge: DALL-E 2 wrapper. 400 char limit. Mood/medium/style dropdowns available.
//
...
// ============================================================================
// COMPLIANCE: Jasper Art-specific cleanup
// ============================================================================

function enforceJasperArtCleanup(text: string): ComplianceResult {
  const fixes: string[] = [];
  let cleaned = text;
...
export function buildJasperArtPrompt(
  _providerId: string,
  ctx: OptimiseProviderContext,
): GroupPromptResult {
  const platformNote = ctx.groupKnowledge ?? '';

  const systemPrompt = `You are an expert prompt optimiser for "Jasper Art". This platform reads natural language prose only. No weight syntax, no parameter flags, no CLIP tokens. Strip all (term:1.3), term::1.3, --ar, --v, --no, "masterpiece", "8K" from input.
...
Return ONLY valid JSON:
{
  ...
  return {
    systemPrompt,
    groupCompliance: enforceJasperArtCleanup,
  };
}
