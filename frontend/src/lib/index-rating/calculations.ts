...
export function calculateMPI(
  ...
  try {
    ...
    // Validate result
    if (!isValidMPI(mpi)) {
      console.warn('[Index Rating] MPI calculation returned invalid value, using default', {
        mpi,
        marketPower: marketPower.foundingYear,
      });
      return DEFAULT_MPI;
    }
...
function calculateSocialFactor(socialReach: ProviderMarketPower['socialReach']): number {
  ...
  const validCounts = platforms.filter((count): count is number => 
 ...
 */
export function calculateNewcomerBoost(
  foundingYear: number,
  foundingMonth: number = 1
): number {
  const now = new Date();
...
export function calculateEffectivePoints(
  ...
  const newcomerBoost = marketPower 
    ? calculateNewcomerBoost(marketPower.foundingYear)
    : 1.0;
...
export function isUnderdog(
  ...
  if (typeof foundingYear === 'number') {
    const now = new Date();
    const monthsOld = 
      (now.getFullYear() - foundingYear) * 12 + now.getMonth();
    if (monthsOld < 12) {
      return true;
    }
  }
...
export function hasRecentRankUp(rankChangedAt: Date | null): boolean {
  ...
  const hoursSinceChange = 
    (now.getTime() - rankChangedAt.getTime()) / (1000 * 60 * 60);
