...
import { PROVIDERS, type ProviderKey, buildAffiliateUrl } from "@/lib/providers";
 ...
 * - Also copies prompt to clipboard when possible.
 */
export async function runAcrossProviders(p: Prompt, only?: ProviderKey[]) {
  ...
  try {
    ...
    // Clipboard might fail silently (permissions).
  }

  const providers = only ? PROVIDERS.filter((x) => only.includes(x.key)) : PROVIDERS;

  // Open synchronously inside the click gesture to avoid popup blockers.
