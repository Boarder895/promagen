...
describe('T_WEIGHT_REBALANCE (weightRebalance)', () => {
  ...
  it('leaves weights alone when already in target range', () => {
    ...
    // Explicit anchors so classifier knows "lighthouse keeper" is the subject
    const anchors: AnchorManifest = {
      ...EMPTY_ANCHORS,
      subjectPhrase: 'lighthouse keeper',
      environmentNouns: ['storm'],
      colours: ['copper'],
    };
    const dna = dnaFor('stability');
...
describe('T_CHAR_ENFORCE (charEnforce)', () => {
  ...
  it('truncates at comma boundary rather than mid-word', () => {
    // Construct a prompt that exceeds ceiling with commas spread throughout
    // so there's always a boundary above the 50% safety floor
    const dna = dnaFor('stability');
    const ceiling = dna.charCeiling;

    // Build segments that each add ~100 chars, ensuring commas above the floor
    const segments: string[] = [];
    let totalLen = 0;
    let segNum = 0;
    while (totalLen < ceiling + 50) {
      const seg = `segment ${segNum} with descriptive visual content padding`;
      segments.push(seg);
      totalLen += seg.length + 2; // +2 for ", "
      segNum++;
    }
    ...
    // Should end cleanly (no mid-word cut) — the last char shouldn't be mid-segment
    expect(result.text).not.toMatch(/segment \d+ with descriptive visual content padd$/);
  });
});
...
describe('classifyVisualCategory (Phase 5 export)', () => {
  ...
  it('classifies quality suffix terms', () => {
    // Note: 8k/4k are in QUALITY_PREFIX_TERMS in attention-sequence.ts (Phase 3).
    // Suffix terms are platform/render markers like artstation, octane render.
    expect(classifyVisualCategory('trending on artstation')).toBe('quality_suffix');
    expect(classifyVisualCategory('octane render')).toBe('quality_suffix');
  });

  it('classifies resolution tags as quality prefix (CLIP training signals)', () => {
    // 8k and 4k are quality_prefix per attention-sequence.ts Phase 3 classification
    expect(classifyVisualCategory('8k')).toBe('quality_prefix');
    expect(classifyVisualCategory('ultra detailed')).toBe('quality_prefix');
  });
