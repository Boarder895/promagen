...
/**
 * /api/crypto — COURIER ONLY (no authority).
 *
 * API Brain rules:
 * - This route MUST NOT decide freshness or eligibility.
 * - It MUST NOT call providers directly.
 * - It MUST call the crypto “authority/client” layer (lib/crypto/providers).
...
import { buildCryptoCacheControl, getCryptoBuildId, getCryptoRequestId } from '@/lib/crypto/route';
import { getCryptoRibbon, toCryptoApiResponse } from '@/lib/crypto/providers';
...
export async function GET(req: NextRequest): Promise<Response> {
  const requestId = getCryptoRequestId();
  const buildId = getCryptoBuildId();
  ...
  const result = await getCryptoRibbon();

  const payload = toCryptoApiResponse({
    requestId,
    buildId,
    generatedAt,
    result,
  ...
  return NextResponse.json(payload, {
    status: 200,
    headers: {
      'Cache-Control': buildCryptoCacheControl({ sMaxAgeSeconds: 300 }),
      'Content-Type': 'application/json; charset=utf-8',
