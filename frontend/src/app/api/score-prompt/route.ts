...
//
// v1.2.0 (2 Apr 2026):
// - Auth enforcement deferred — needs Clerk middleware verification first.
//   Client-side Lab Gate prevents free users from reaching Call 4.
...
// - Harsher scoring philosophy in system prompt.
...
export async function POST(req: NextRequest) {
  // ── Auth: Pro enforcement deferred ───────────────────────────────
  // TODO: Add server-side Clerk auth check once middleware config is
  // verified for /api/score-prompt. Client-side Lab Gate prevents
  // free users from reaching Call 4 (fires only after Call 3, which
  // requires Pro). Server-side check is belt-and-braces for later.
