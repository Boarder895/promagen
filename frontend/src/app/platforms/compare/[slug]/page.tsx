...
function RelatedComparisons({ currentSlug }: { currentSlug: string }) {
  ...
  // Pre-resolve names outside the render loop
  const resolved = related.map((r) => ({
    slug: r.slug,
    aName: getAuthorityPlatformById(r.platformAId)?.name ?? r.platformAId,
    bName: getAuthorityPlatformById(r.platformBId)?.name ?? r.platformBId,
  }));
