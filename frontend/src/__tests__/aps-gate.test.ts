...
describe('APS Gate — Critical anchor veto', () => {
  test('subject dropped → REJECT via Veto 1 even from RETRY band', () => {
    const anchors = makeAnchors();
    ...
    // Architecture §6.3: "reject regardless of APS score" → veto overrides to REJECT
    expect(result.scoreVerdict).toBe('RETRY');
