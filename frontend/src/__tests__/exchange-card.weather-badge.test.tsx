...
describe('ExchangeCard – weather badge', () => {
  ...
  it('renders a weather badge when weather is provided', () => {
  ...
  it('does not render a weather badge when weather is missing', () => {
    render(<ExchangeCard exchange={baseExchange} />);

    expect(screen.queryByTestId('exchange-weather-badge')).not.toBeInTheDocument();
  });
});
