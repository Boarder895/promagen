Promagen Fly.io Brain – Deployment & Cost Model (v1) 0. Goal
...
Serve your /api/\* endpoints to the frontend.

Example routes Promagen might expose via Fly.io:

/api/fx/live – hitting your FX provider(s) for ribbon + mini-widget.
...
Compute (Fly Machines / VMs)

Billed based on vCPU, RAM, and uptime.

Auto-scaling and idle-shutdown can reduce cost by stopping machines at low traffic periods.

Storage (persistent volumes)

Billed roughly per GB per month (e.g. on the order of ~$0.15/GB-month in typical analyses).

Bandwidth / egress

Outbound data to users and external services is billed per GB (example ballpark: ~$0.02/GB in Europe/North America; exact numbers depend on region).

Support / service tiers

Optional standard support plans (e.g. around $29/month) for additional guarantees and help.

Legacy vs new billing

Older accounts may have access to a legacy “free tier” or low flat plans; newer accounts are primarily usage-based.
...
Machine size (vCPU/RAM) and uptime for your backend workers and API routes.

Multiple regions = multiple sets of machines and cost per region.
...
Volume size × price/GB/month.

Bandwidth / egress

Data sent from Fly.io to your users (dashboards, live updates).

Data sent to other services.

Inter-region transfers if you run multiple regions (e.g. London ↔ Singapore).

Support / premium services

Optional if you want SLAs or higher support responsiveness for paid users.

Regions / redundancy

Global footprint (e.g. London + New York + Singapore) gives better latency but multiplies compute cost.

Scale / growth

More users → more traffic → more compute, storage, and egress.

Costs scale fairly linearly, especially if you optimise caching and batch calls.
...
Realistic total: £20–£55/month. Promagen is cheap to run here – ideal for your first public version.
...
Realistic total: £100–£180/month. This is when Promagen feels “alive” and responsive for a global-ish audience.
...
Realistic total: £350–£1,100/month – still modest for a global finance-style platform and usually cheaper than an equivalent AWS setup.
...
7. How this ties back to the API brain

Fly.io is where the API brain becomes a running organism:

api.providers.catalog.json + api.endpoints.catalog.json tell your Fly workers who to call and what to call.

api.policies.json + quota logic tell them how often to call each role safely.

Fly.io hosts:

the worker that reads those catalogues,

the quota helper that enforces the 31-day safety maths,

the cache layer that keeps data warm for your ribbons and mini widgets.
