...
The rule of thumb is scope. If an animation is used by one component only, it belongs in that component file (either as a <style jsx> block in the component, or Tailwind's @keyframes in a component-scoped CSS module). If it's shared across multiple components or applied via className from different places, globals is the right home.
In your case, your leaderboard animations (sortable headers, provider cells, vote thumbs, market pulse, tick arrows) are all applied via className strings on elements rendered by different components, so globals.css is correct for those. But if you ever had a one-off animation used only inside, say, engine-bay.tsx, that should live in that component's <style jsx> — which you already do correctly for Engine Bay's shimmer animations.
The risk of putting everything in globals is bloat — every page loads every animation even if that page doesn't use the leaderboard. For Promagen that's fine because your leaderboard is on the homepage which is your main page, and the total CSS is modest. It would only matter if you had dozens of page-specific animations.
