# Inspire Page — Authority Document

> **Route:** `/inspire` (was `/` until Prompt Lab promotion, v8.0.0)
> **Note:** The homepage (`/`) is now the Prompt Lab. This doc describes the Inspire page at `/inspire`.
...
The new homepage replaces the current financial-data-first layout with a prompt-first experience. A first-time visitor must understand what Promagen does within 3 seconds: **it builds intelligent image prompts for 40 AI platforms.**
...
| "40 AI Image Generators — Elo-Ranked" heading   | Visible                | **Kept**                                                                            |
...
├──────────────┼──────────────────────────────────────────┼───────────────────┤
│              │                                          │                   │
│  SCENE       │  40 AI Image Generators — Elo-Ranked     │  COMMUNITY        │
...
2. "40 AI Image Generators" heading   (existing LeaderboardIntro)
...
- **Leaderboard Bridge:** Active tier colour flows from showcase → heading underline → table row highlighting → filter chip. `onTierChange` callback from `CityContent` → `PromptShowcase` → `new-homepage-client.tsx`. Tier accent: 2px underline on "40 AI Image Generators" heading. Row highlighting: 3px left border + 3% tint on matching providers. Filter: subtitle swaps between "Click ▼ Provider to expand" + "Filter: {Tier} (N)" and "{Tier} · N of 40 · Clear".
...
**Demo prompts (pre-user-traffic):** 210 pre-generated prompts stored in `src/data/community-pulse/demo-prompts.json`. 5 prompts per platform across all 40 platforms. Each prompt is assembled and optimised for its specific platform format (T1 CLIP weights, T2 Midjourney params, T3 Natural Language sentences, T4 Plain Language). Prompts built from real vocabulary files — coherent visual scenes, not random term combinations. Every prompt fills 85-99% of its platform's `idealMax` character budget.

**Demo prompt data per entry:** `platformId`, `platformName`, `platformIcon`, `brandColor`, `description`, `optimisedPrompt`, `score`, `countryCode`, `locationName` (real town per country — 5 unique towns per country across 40 countries), `likeCount`. No `localTime` field — replaced by `locationName` in v9.0.0.
...
| (40 platforms total — see `PLATFORM_COLORS` map in component) |              |         |
...
- Right: "40 platforms" in `text-emerald-400`
...
| File                                              | Purpose                                            | Size       |
| ------------------------------------------------- | -------------------------------------------------- | ---------- |
| `src/data/community-pulse/demo-prompts.json`      | 210 pre-generated demo prompts (5 x 40 platforms)  | ~130 KB    |
...
- [x] 210 demo prompts displayed in 8-card rotation (5 per platform x 40 platforms)
...
- **Demo prompts: 210 entries** — 5 per platform x 40 platforms. If platforms are added/removed, regenerate via `npx tsx scripts/generate-demo-prompts.ts`.
...
- **9 March 2026 (v7.0.0):** **SCENE STARTERS 100% ENRICHMENT + CONFLICT ENGINE v2 + COMMODITY CARD v3.0.**
  ...
  **Scene Starters — all 200 scenes now score 100% on all 40 platforms (4 tiers):**
...
- **7 March 2026 (v6.0.0):** **UNIFIED FEEDBACK SYSTEM + LEADERBOARD BRIDGE + PROMPT SHOWCASE v11.**
  ...
  - **Leaderboard Bridge (Ideas 1+3+5):** Tier accent underline on "40 AI Image Generators" heading (colour follows active tier, 400ms transition). Provider row highlighting (3px left border + 3% tint on tier-matching providers). Filter system: subtitle shows "Click ▼ Provider to expand · Filter: CLIP-Based (7)" — clicking filters table. Active filter: "{Tier} · N of 40 · Clear". State lifted to `new-homepage-client.tsx` via `showcaseTierId` + `tierFilter`.
...
- **6 March 2026 (v5.0.0):** **PROMPT SHOWCASE v10 HERO REDESIGN + COMMUNITY PULSE v9.0 + DEMO PROMPT BUGFIXES + PROVIDER LINK FIX.** Major overhaul of centre column and right rail:
  ...
  - **Community Pulse v8→v9:** Extended from 6 to 8 platform prompt cards. Online users cards removed. Footer: "40 platforms".
  ...
  - **Provider links fixed:** `/go/[providerId]` route updated for Next.js 15 async params (`Promise<{ providerId: string }>`). Test file updated with `Promise.resolve()` wrappers. All 40 provider homepage links now work.
...
- **6 March 2026 (v4.0.0):** **COMMUNITY PULSE v8.0.0 COMPLETE REDESIGN + BUILDER PIPELINE WIRING.** §6 rewritten completely:
  ...
  - **210 demo prompts:** 5 per platform x 40 platforms. Built from real vocabulary files. Each prompt optimised for its specific platform format (T1 CLIP weights + quality prefix, T2 MJ params, T3 NL sentences, T4 plain). All fill 85-99% of platform idealMax. Coherent visual scenes (samurai in rain, coral reef, jazz musician, etc.) not random word soup.
