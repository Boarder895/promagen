...
> **Cross-references:**
>
> - `ai-disguise.md` v3.0.0 — Full AI Disguise specification (system prompt, post-processing, harmony engineering)
> - `harmonizing-claude-openai.md` v1.0.0 — Claude↔GPT system prompt engineering playbook
...
> - `prompt-lab.md` v3.1.0 — Prompt Lab architecture
> - `paid_tier.md` — Pro gating, free/paid boundaries (the only place that defines what is free vs paid)
> - `optimal-prompt-stacking.md` v2.0.0 — Per-platform stacking limits (45 platforms × 12 categories)
...
11. [UK Compliance (ASA/CAP)](#11-uk-compliance-asacap)
12. [Canonical Outbound Rule](#12-canonical-outbound-rule)
13. [Affiliate Schema & Registry](#13-affiliate-schema--registry)
14. [Redirect Endpoint `/go/[providerId]`](#14-redirect-endpoint-goproviderid)
15. [Click Logging & Privacy](#15-click-logging--privacy)
16. [Disclosure Wording](#16-disclosure-wording)

**Part C — AI Disguise (Summary)**
17. [AI Disguise Overview](#17-ai-disguise-overview)

**Part D — Governance**
18. [File Map](#18-file-map)
19. [Testing & Lock-in Proofs](#19-testing--lock-in-proofs)
20. [Known Gaps](#20-known-gaps)
21. [Non-Regression Rules](#21-non-regression-rules)
...
| Platform formats | `src/data/providers/platform-formats.json` | Assembly rules per platform (promptStyle, sweetSpot, tokenLimit, etc.) |
| Platform tiers | `src/data/platform-tiers.ts` | 4-tier assignments (T1: 13, T2: 2, T3: 14, T4: 16) |
| Provider capabilities | `src/data/providers/providers.capabilities.json` | Per-provider feature flags |
| Prompt limits | `src/data/providers/prompt-limits.json` | maxLength per platform |
...
| Compression support | `src/data/compression/platform-support.json` | Compression profiles per platform |
...
| ---- | ---- | ------------ | -------------- |
| **T1** | CLIP-Based | Weighted keywords `(term:1.3)`, stacking, quality prefix/suffix | 13 |
| **T2** | Midjourney Family | Prose with `::` weighting, `--ar`, `--v`, `--no` parameters | 2 |
| **T3** | Natural Language | Conversational sentences, no special syntax | 14 |
| **T4** | Plain Language | Simple, short, focused — minimal jargon | 16 |

### Platform assignments (from `platform-tiers.ts`)

**Tier 1 — CLIP-Based (13):**
`artguru`, `clipdrop`, `dreamlike`, `dreamstudio`, `getimg`, `leonardo`, `lexica`, `nightcafe`, `novelai`, `openart`, `playground`, `stability`, `tensor-art`

**Tier 2 — Midjourney Family (2):**
`bluewillow`, `midjourney`

**Tier 3 — Natural Language (14):**
`adobe-firefly`, `bing`, `flux`, `google-imagen`, `hotpot`, `ideogram`, `imagine-meta`, `jasper-art`, `kling`, `luma-ai`, `microsoft-designer`, `openai`, `recraft`, `runway`

**Tier 4 — Plain Language (16):**
`artbreeder`, `artistly`, `canva`, `craiyon`, `deepai`, `fotor`, `freepik`, `myedit`, `photoleap`, `picsart`, `picwish`, `pixlr`, `simplified`, `visme`, `vistacreate`, `123rf`
...
| Canva | T4 | None (simple input) | Converted to positive |
...
The Prompt Lab (`/studio/playground`) uses three API calls to GPT-5.4-mini, presented to the user as algorithmic processing. **Full specification in `ai-disguise.md` v3.0.0.**
...
| Call 2 | `POST /api/generate-tier-prompts` | Generate 4 native tier prompts (18-rule system prompt + post-processing) | Tier cards populate |
| Call 3 | `POST /api/optimise-prompt` | Optimise assembled prompt for specific provider | Algorithm cycling (101 names) |
...
All 45 providers are tier-aware. When a provider is selected, Call 2's system prompt receives provider context with exact weight syntax, sweetSpot, tokenLimit, qualityPrefix, and negativeSupport. The 18-rule system prompt enforces provider-specific output. Post-processing catches mechanical GPT artefacts. See `ai-disguise.md` §6–§8 and `harmonizing-claude-openai.md` for methodology.
...
| `src/data/providers/platform-formats.json` | Assembly rules | 45 platform configs |
| `src/data/providers/providers.capabilities.json` | Feature flags | 45 |
| `src/data/providers/prompt-limits.json` | Length limits | 45 |
...
| `src/data/platform-tiers.ts` | Tier assignments | 4 tiers (13+2+14+16) |
...
| `src/components/providers/describe-your-image.tsx` | Human text input | 722 |
| `src/components/prompt-builder/four-tier-prompt-preview.tsx` | 4-tier cards | 788 |
...
### Tier system
6. **Tier assignments in `platform-tiers.ts` are authoritative** — do not duplicate in other files
7. **Weight syntax is provider-specific** — Leonardo uses `::`, SD uses `()`, when no provider selected default to parenthetical
8. **Jasper Art is Tier 3** — not Tier 1 (reclassified 19 March 2026)
9. **Luma AI has no negative prompt support** — `negativeSupport: 'none'`

### AI Disguise
10. **Full AI Disguise specification lives in `ai-disguise.md`** — this doc contains summary only
