...
import {
  ArrowUpRight, ArrowDownRight, Search, Filter, ExternalLink,
  Lock, Copy, ClipboardCheck, ChevronLeft, TrendingUp,
} from "lucide-react";
...
const DEFAULTS: Provider[] = [
  { id: "openai", name: "OpenAI", site: "openai.com",
    autoScore: 87.3, manualAdjustment: 0, hardOverrideScore: null, delta: 1.4,
    hasApi: true, category: "Image",
    tagline: "Fast multi-modal models and robust APIs.",
    affiliateUrl: "/out/openai" },
  { id: "leonardo", name: "Leonardo AI", site: "leonardo.ai",
    autoScore: 84.1, manualAdjustment: 0, hardOverrideScore: null, delta: -0.6,
    hasApi: true, category: "Image",
    tagline: "High-quality models and community assets.",
    affiliateUrl: "/out/leonardo" },
  { id: "midjourney", name: "Midjourney", site: "midjourney.com",
    autoScore: 81.9, manualAdjustment: 0, hardOverrideScore: null, delta: 0.2,
    hasApi: false, category: "Image",
    tagline: "Exquisite aesthetics (Discord workflow).",
    affiliateUrl: "/out/midjourney" },
  { id: "canva", name: "Canva", site: "canva.com",
    autoScore: 76.4, manualAdjustment: 0, hardOverrideScore: null, delta: 0.8,
    hasApi: false, category: "Design Suite",
...
function rankBadge(rank: number) {
  ...
  if (rank <= 5) return <Badge variant="secondary" className="rounded-2xl">Top 5</Badge>;
  return <Badge variant="outline" className="rounded-2xl">#{rank}</Badge>;
}
...
function Sparkline({ trend = [74, 75, 76, 75.5, 76.4] }: { trend?: number[] }) {
  const path = React.useMemo(() => {
    const w = 100, h = 28, max = Math.max(...trend), min = Math.min(...trend);
  ...
  return <svg viewBox="0 0 100 28" className="w-24 h-7">
...
function ProviderRow({
  i, p, onOpen,
}: { i: number; p: Provider; onOpen: (p: Provider) => void; }) {
  const up = p.delta >= 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <img src={faviconFor(p.site)} alt="" className="size-9 rounded-xl border" />
      ...
      <div className={clsx("flex items-center gap-1 justify-end font-medium tabular-nums",
        up ? "text-emerald-600" : "text-rose-600")}>
        {up ? <ArrowUpRight className="size-4" /> : <ArrowDownRight className="size-4" />}
        {up ? "+" : ""}{p.delta.toFixed(1)}
      </div>
      <div className="justify-self-end"><Sparkline /></div>
    </motion.button>
  );
}
...
function ProviderView({ p, onBack }: { p: Provider; onBack: () => void }) {
  ...
  function openAffiliateWrapped() { window.open(p.affiliateUrl, "_blank", "noopener,noreferrer"); }
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <CardTitle className="text-2xl flex items-center gap-3">
                  <img src={faviconFor(p.site)} alt="" className="size-10 rounded-2xl border"/>
                  {p.name}
                  <Badge variant="secondary" className="rounded-2xl">{p.category}</Badge>
                </CardTitle>
              ...
              <div className="text-right">
                ...
                <div className={clsx(
                  ...
                  {p.delta>=0? <ArrowUpRight className="size-4"/>: <ArrowDownRight className="size-4"/>}
                  {p.delta>=0? "+":""}{p.delta.toFixed(1)} today
                </div>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Button onClick={handleCopy} className="rounded-2xl">
                  {copied ? (<><ClipboardCheck className="mr-2 size-4"/>Copied</>) :
                    (<><Copy className="mr-2 size-4"/>Copy Prompt</>)}
                </Button>
                <Button onClick={openAffiliateWrapped} variant="secondary" className="rounded-2xl">
                  <ExternalLink className="mr-2 size-4"/>Open {p.name} in Promagen Window
                </Button>
        ...
        <Card className="rounded-3xl">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><TrendingUp className="size-5"/> Live Rank Signals</CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl border p-3"><div className="text-xs opacity-60">Adoption/Ecosystem</div><div className="text-xl font-semibold">86</div></div>
              <div className="rounded-2xl border p-3"><div className="text-xs opacity-60">Image Quality</div><div className="text-xl font-semibold">89</div></div>
              <div className="rounded-2xl border p-3"><div className="text-xs opacity-60">Speed/Uptime</div><div className="text-xl font-semibold">82</div></div>
              <div className="rounded-2xl border p-3"><div className="text-xs opacity-60">Cost/Free Tier</div><div className="text-xl font-semibold">74</div></div>
              <div className="rounded-2xl border p-3"><div className="text-xs opacity-60">Trust/Safety</div><div className="text-xl font-semibold">91</div></div>
              <div className="rounded-2xl border p-3"><div className="text-xs opacity-60">Automation/Innovation</div><div className="text-xl font-semibold">88</div></div>
              <div className="rounded-2xl border p-3"><div className="text-xs opacity-60">Ethical/Environmental</div><div className="text-xl font-semibold">71</div></div>
            </div>
            <p className="text-xs opacity-70">Scores are normalized 0–100 with evidence-based sources; movement debounced.</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="mt-6 rounded-3xl">
        <CardHeader>
          ...
          <CardDescription>Optional upgrade path — consistent UI for all providers.</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-wrap items-center gap-3">
          <Button variant="outline" disabled className="rounded-2xl">
            <Lock className="mr-2 size-4"/> Use via API (Coming Soon)
          </Button>
          <p className="text-sm opacity-70">When enabled, prompts send directly via your saved API key and results appear inside Promagen.</p>
        </CardContent>
      </Card>
    </div>
  );
...
function LeaderboardView({
  providers, onOpen, onAdmin,
}: { providers: Provider[]; onOpen: (p: Provider) => void; onAdmin: () => void }) {
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="rounded-2xl">AI Tools Ranked Live</Badge>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" className="rounded-2xl" onClick={onAdmin}>Admin: Scores</Button>
          <div className="text-xs opacity-70">Affiliate disclosure: we may earn commissions on outbound links.</div>
        </div>
      </div>
      ...
      <Card className="rounded-3xl">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Search providers…" className="pl-9 rounded-2xl w-64"
              />
            </div>
        ...
        <CardContent>
          <div className="divide-y">
            {filtered.map((p, i) => (<ProviderRow key={p.id} i={i} p={p} onOpen={onOpen} />))}
          </div>
        </CardContent>
      </Card>
    </div>
...
}: { providers: Provider[]; onClose: () => void; onSave: (id: string, patch: Partial<Provider>) => void }) {
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={commit} className="rounded-2xl">Save Changes</Button>
        </div>
      </div>
      ...
      <Card className="rounded-3xl">
        <CardHeader>
          ...
          <CardDescription>FinalScore = hardOverride ?? clamp(autoScore + manualAdjustment)</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-[1fr_140px_160px_160px_120px] gap-3 items-center">
            ...
            <div className="text-xs uppercase tracking-wide opacity-60 text-right">Hard Override</div>
            <div className="text-xs uppercase tracking-wide opacity-60 text-right">Δ Today</div>
          </div>
          <div className="divide-y mt-2">
            {draft.map(p => (
              <div key={p.id} className="grid grid-cols-[1fr_140px_160px_160px_120px] gap-3 items-center py-2">
                <div className="flex items-center gap-3">
                  <img src={faviconFor(p.site)} alt="" className="size-8 rounded-lg border" />
                  <div className="leading-tight">
                    <div className="font-medium">{p.name}</div>
                    <div className="text-xs opacity-60">{p.site}</div>
                  </div>
                </div>
                <Input type="number" step="0.1" value={p.autoScore}
                  onChange={e => setField(p.id, 'autoScore', parseFloat(e.target.value))}
                  className="text-right" />
                <Input type="number" step="0.1" value={p.manualAdjustment}
                  onChange={e => setField(p.id, 'manualAdjustment', parseFloat(e.target.value))}
                  className="text-right" />
                <Input type="number" step="0.1" value={p.hardOverrideScore ?? ''} placeholder="(blank = none)"
                  onChange={e => setField(p.id, 'hardOverrideScore', e.target.value === '' ? null : parseFloat(e.target.value))}
                  className="text-right" />
                <Input type="number" step="0.1" value={p.delta}
      ...
      <p className="text-xs opacity-70">
        MVP persistence: these values save to localStorage. In production, wire this to your DB with admin auth
        and feed <em>autoScore</em> from your collectors; apply debouncing and snapshots.
      </p>
    </div>
  );
}
...
export default function PromagenMVP() {
  const [providers, setProviders] = React.useState<Provider>(() => {
    ...
    return DEFAULTS;
  }) as unknown as React.useState<Provider[]>[0]; // TSX single-file quickfix

  const [active, setActive] = React.useState<Provider | null>(null);
  const [admin, setAdmin] = React.useState(false);

  React.useEffect(() => {
    try { localStorage.setItem('promagen.providers', JSON.stringify(providers)); } catch {}
  }, [providers]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900 text-zinc-900 dark:text-zinc-50">
      {admin ? (
        <AdminScores providers={providers} onClose={() => setAdmin(false)} onSave={updateProvider} />
      ) : active ? (
