...
async function collectPromagenUsage(providerId: string) {
  ...
  const [{ active_users }] = await prisma.$queryRawUnsafe<{ active_users: number }[]>(
    `
  ...
  const [{ gens_min }] = await prisma.$queryRawUnsafe<{ gens_min: number }[]>(
    `
  ...
  const [{ p50_ms, p95_ms, success_rate }] =
    await prisma.$queryRawUnsafe<{ p50_ms: number; p95_ms: number; success_rate: number }[]>(
      `
      WITH w AS (
